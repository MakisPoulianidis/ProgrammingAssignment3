rankhospital <- function(state, outcome, num="best") {
        ...
        if (outcome=="heart attack") {
                ...
                if (num=='worst')
                {
                        rank_output <-transform(sort_output, rank = rank(-sort_output$heart_attack, ties.method = 'first'))
                ...
                if (num=='worst')
                {
                        rank_output <-transform(sort_output, rank = rank(-sort_output$pneumonia, ties.method = 'first'))
                ...
                if (num=='worst')
                {
                        rank_output <-transform(sort_output, rank = rank(-sort_output$heart_failure, ties.method = 'first'))
        ...
        if (1>hospital.number)
        {
                ##print("1>hospital.number")
                return(NA_character_)
        }
        else if (nrow(rank_output)<hospital.number)
        {
                ##print("nrow(rank_output)<hospital.number")
                return(NA_character_)
        }
        else
        {
        z <- subset(rank_output,rank_output$rank==hospital.number, select=c(1:6 ))
